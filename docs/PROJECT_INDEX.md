...
> 最后更新：2026-07-09（终检：12文档答辩数据体系 + 文档同步完成）
...
### `lab_test_plan.md` 🆕
**明日实验室测试验证计划**（约 200 行，组长A撰写，B+A执行）。包含：环境准备清单、4项测试的依赖排序（ALU→regfile→control→CPU基础程序）、每项测试的具体测试向量和通过标准、常见问题应急预案、测试记录模板、完成后输出清单。目标：在 Vivado 2018.3 + xsim 下验证 RV32I 多周期 FSM CPU 核心功能。
...
内容：记录模板、记录要求、10 条已记录的使用（AI-20260706-01~03：项目资料整理+MVP文档+分工协作；AI-20260707-01~03：约束核对+协作文档+文件索引；AI-20260708-01~05：六仓库分析+深度合并生成+文档同步+可行性分析+AI声明表）。每条含日期/成员/模块/工具/提示词摘要/输出摘要/验证方式/是否合并。**每次 AI 使用必须追加。**

### `ai_declaration.md` 🆕
**AI 辅助工具使用声明表**（约 300 行，组长A代表全组填写）。依据课程要求（①需求与指令集/接口定义 ②参考代码工程审阅与改造说明 ③硬件平台落地调试 ④系统集成与联调验收），逐项填写。包含：15个模块的逐模块改造说明（参考来源、原始内容、改造内容、改造原因、独立设计占比百分比）、学术诚信承诺、每位成员独立讲解能力确认表。
...
**五仓库深度合并方案**（约 400 行）。基于五个开源仓库（NCUT_MiniSys / SUSTech CS202 / SEU minisys / riscv-minisys-cpu / minisys_unified）的深度分析，制定的完整合并蓝图。包含：架构选型（P0多周期FSM→P1流水线→P2 BTB）、组件来源决策表（CPU/总线/外设从哪个仓库借鉴）、MIPS→RV32I 改造路径（编码/控制信号/ALU映射）、合并后完整目录结构、6阶段实施计划、统一接口规范、风险控制与降级方案、关键技术决策记录、参考仓库文件对照速查表。

### `integration_report.md` 🆕
**五仓库深度合并整合报告**（约 500 行，组长A撰写）。记录组长A完成的代码整合工作全过程。包含：六仓库选型分析（每个仓库的可取之处和不可用原因）、合并过程中的4个关键设计决策（总线地址空间选择/ALU操作码扩展/多周期FSM状态设计/CPU_MODE参数化）、一致性验证报告（板级约束100%一致/ISA编码完全一致/接口满足规范/内存映射一致）、24个RTL文件的生成追溯与改造程度分类、当前项目功能清单。**本报告是本项目"深度整合"而非"简单复制"的核心证据**，体现组长A的架构选型思考过程。

### `optimization_roadmap.md` 🆕
**拓展方向可行性分析与优化路线图**（约 350 行，组长A撰写）。基于 Minisys XC7A100T 硬件资源约束，对课程六个拓展方向逐一分析可行性+硬件开销+预期收益+与现有基础的契合度：①流水线冒险(⭐⭐⭐⭐⭐推荐) ②Cache(⭐⭐⭐中) ③浮点/乘除(⭐⭐低ROI) ④自定义ISA(⭐⭐⭐中，MAC已实现) ⑤MAC加速(⭐⭐⭐⭐⭐推荐，已完成基础) ⑥PPA对比(⭐⭐⭐⭐贯穿所有方向)。给出推荐优先级排序、一周优化路线图（Day1-2流水线→Day3-4 PPA+MUL/DIV→Day5-6 BTB冲刺→Day7报告）、答辩主线建议。包含 Artix-7 资源预算估算表。
...
| `riscv_pipeline_cpu.v` | 🆕 ✅ 已生成 | A | ★ RV32I五级流水线CPU：IF→ID→EX→MEM→WB，forwarding+load-use stall+branch/JAL/JALR flush |
| `riscv_pipe_wrapper.v` | 🆕 ✅ 已生成 | A | 流水线CPU→统一总线适配wrapper |
| `pipeline/btb.v` | 🆕 ✅ 已生成 | A+AI | **BTB分支目标缓冲**：16条目直接映射+2-bit饱和计数器（Smith预测器），IF查找+EX更新，含统计计数器 |
| `pipeline/br_predictor.v` | 🆕 ✅ 已生成 | A+AI | 分支预测精度追踪器：BTB包装器+分支统计（总分支/误预测/BTB命中） |
| `cpu_top.v` | ✅ 已生成 | A | CPU模式选择器：generate块按CPU_MODE参数实例化不同CPU（支持MODE=0~5） |

### `src/bus/` — 总线系统（借鉴SEU minisys）
...
| 目录 | 负责人 | 已提交文件 | 状态 |
|---|---|---|---|
| `sim/tb/` | B/D/A | `tb_alu.v`, `tb_regfile.v`, `tb_control_unit.v`, `tb_cpu_basic.v`（B xsim通过）+ `tb_mac.v`, `tb_perf_counter.v`, `tb_perf_integration.v`, `tb_dot_product.v`, `tb_perf_mmio.v`（D Icarus通过）+ `tb_pipeline_basic.v` 🆕（A，流水线CPU仿真） | ✅ 10 个 testbench |
...
| `tests/branch/` | B | — | TODO |
| `tests/mmio/` | C | — | TODO |
| `tests/hazard/` | A | `hazard_test.hex` 🆕（24条指令，覆盖RAW forwarding/load-use stall/branch taken+not-taken/JAL flush/JALR flush） | ✅ 1 组冒险测试 |
| `tests/pipeline/` 🆕 | A+AI | `branch_loop_test.S`（10次循环BTB训练测试+行为分析注释） | ✅ 1 个BTB测试 |
| `sim/tb/` | B+D+A | `tb_pipeline_basic.v` + `tb_pipeline_btb.v` 🆕 + 其他 9 个 testbench | ✅ 11 个 testbench |
| `sim/programs/` | A+AI | `basic_test.hex` + `branch_loop_test.hex` 🆕（10次循环分支测试） | ✅ 2 个测试程序 |
| `reports/tables/` | D+A | `perf_comparison.md`, `ppa_comparison.md`, `test_results.md`, `pipeline_hazard_analysis.md`, `branch_prediction_stats.md`, `pipeline_ppa_estimates.md`, `performance_summary.md` 🆕, `cpi_breakdown.md` 🆕, `resource_utilization.md` 🆕, `speedup_analysis.md` 🆕, `before_after_comparison.md` 🆕, `performance_charts.md` 🆕 | ✅ 12 个报告 |
| `reports/vivado/` | B/C | `README.md`（归档要求） | 🔄 待填实际数据 |
| `reports/figures/` | A+AI | `pipeline_performance_dashboard.html` 🆕（KPI+CPI对比+流水线时空图+BTB状态机+冒险方案表） | ✅ 1 个可视化 |
| `figures/` | B | `connect-target.png`, `generate-bitstream.png`, `implementation.png`, `reset-runs.png`, `synthesis.png`（Vivado 操作流程截图） | ✅ 5 张截图 |
| `scripts/` | C | — | TODO |
