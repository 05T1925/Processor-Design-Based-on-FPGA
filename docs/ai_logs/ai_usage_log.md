...
## 2026-07-13 README 终版整理与复现说明同步

- 日期：2026-07-13
- 工具：Codex（GPT-5）+ PowerShell + Git
- 工作内容：
  - 以当前 RTL、`cpu_guess_game.S`、构建 Tcl、约束文件和 `build_cpu_vga` 实现报告为依据，完善中文 README。
  - 新增直接烧录、从源码生成 ROM/bitstream、快速定位、可验收亮点、测试通过标准、报告引用边界和已知限制。
  - 逐项复核按键语义：猜数字为 S1/S2 切换位、S3/S4 增减、S5 提交；性能页为 S1/S2 选基准、S5 运行；Tetris 为 S1 左、S2 右、S3 软降/暂停时重开、S4 旋转、S5 暂停/继续。
  - 修正旧 README 中猜数字按键说明与汇编实际实现不一致的问题，并同步章节引用。
- 证据边界：
  - 多周期 VGA SoC 的实现报告直接来自 `build_cpu_vga/`；流水线具体 PPA 继续要求引用负责队友的同次 `CPU_MODE=5` implementation 报告。
  - 本次未改动 RTL、测试或 Vivado 生成物，仅优化交付文档并进行一致性检查。
- 是否合并：提交 README 和本条 AI 日志并推送。
