...
| 跨目录引用 | `src/soc/soc_top.v` | `` `include "public.vh" `` | ⚠️ **需要设置include搜索路径** |

**解决方案**（二选一，推荐方案A）：

**方案A（推荐）**：在 Vivado 工程中设置全局 Include 路径
```
Project Settings → General → Verilog Options → Verilog Include Files Search Paths
添加: src/core/
```

**方案B**：修改 `soc_top.v` 的 include 为相对路径
```verilog
// 改前
`include "public.vh"
// 改后
...
| 　分支预测(BTB) | SEU minisys BTB.v已分析 | 🔄 P2 | 2028行参考代码 |
...
| 引脚功能 | 本项目 minisys.xdc | SUSTech CS202 | SEU minisys | —（同一仓库）| 一致性 |
|---|---|---|---|---|---|
...
| 2 | 添加所有 Verilog 源文件 | `src/core/*.v`, `src/bus/*.v`, `src/memory/*.v`, `src/io/*.v`, `src/common/*.v`, `src/soc/*.v`, `src/board/*.v` |
| 3 | 设置 global include 路径 | 添加 `src/core/`（解决 `public.vh` 跨目录include） |
| 4 | 添加约束文件 | `constraints/minisys.xdc` |
| 5 | 设置顶层模块 | `minisys_top` |
| 6 | （可选）`define MINISYS_USE_SOC_TOP | 在 Vivado `Verilog Options` → `Define Macros` 中设置 |
...
### 4.3 已知的注意事项（2026-07-09 更新：D 已修复4项，新增1项）

| 问题 | 严重程度 | 状态 | 说明 |
|---|---|---|---|
| ~~MAC 写回组合引用 `mac_result` 形成组合环~~ | ~~⚠️ 中~~ | ✅ 已修复 | D修复：EXECUTE锁存到`alu_result`，WRITEBACK写回锁存值 |
| ~~taken branch 使用上一周期 `branch_taken`~~ | ~~⚠️ 高~~ | ✅ 已修复 | D修复：EXECUTE直接使用当前`br_taken`选择PC |
| ~~instret 只在 WRITEBACK 计数~~ | ~~⚠️ 中~~ | ✅ 已修复 | D修复：STORE→MEMORY、BRANCH→EXECUTE、寄存器写回→WRITEBACK |
| ~~MAC 只检查 opcode 不检查 funct3/funct7~~ | ~~⚠️ 中~~ | ✅ 已修复 | D修复：严格检查`funct3=000`且`funct7=0000001` |
| 现有 Vivado 综合报告仅 heartbeat（2 LUT / 24 FF） | 🔴 严重 | 待解决 | B/C 需重跑完整 SoC 综合（不含 `MINISYS_USE_HEARTBEAT`），两版本（基线/MAC） |
| `$clog2` 综合警告 | ⚠️ 低 | 监控中 | 若有警告，替换为 `localparam ADDR_WIDTH = 13;` |
| P20复位极性 | ⚠️ 中 | 待上板 | `.xdc` 中无极性说明，需上板实测 |
...
Vivado 2017.4 兼容性    ✅ 通过 — $clog2已替换为可综合函数，其余语法完全兼容
课程基础层次对齐         ✅ 100% — 综合/实现/bitstream全部通过(WNS=7.212ns, DRC=0)
课程进阶层次对齐         ✅ 85% — SoC框架+perf MMIO+CPI/吞吐量量化完成，待流水线+完整SoC综合
课程拓展层次对齐         ✅ 80% — MAC验证+点积对比(speedup=1.1481)+PPA模板完成，待Vivado DSP推断+流水线
板级约束一致性          ✅ 100% — 四仓库.xdc交叉验证通过，配置电压已补充
RTL控制通路缺陷         ✅ 已修复 — MAC组合环/branch_taken/instret退休/MAC非法编码共4项（D PR #2）
```

### 综合/实现验证结果（⚠️ 需注意：当前仅 heartbeat，需重跑完整 SoC）

```
现有报告 (heartbeat, 无效):  LUT=2, FF=24, DSP=0, WNS=+7.212ns
需要的报告 (完整SoC):        LUT/FF/BRAM/DSP utilization + timing (基线 + MAC 两版本)
负责人: B/C (Vivado 2018.3)
```

### 实验室验证路径

```
1. ✅ Vivado建工程 → 添加源文件 → 添加.xdc（B完成）
2. ✅ xsim跑 tb_alu tb_regfile tb_control_unit tb_cpu_basic（B完成）
3. ✅ Icarus跑 tb_mac tb_perf_counter tb_perf_integration tb_dot_product tb_perf_mmio（D完成）
4. ✅ Verilator lint soc_top → 无error，无组合环（D完成）
5. ⚠️ Synthesis → 现有报告仅heartbeat（2 LUT/24 FF），需重跑完整SoC
6. ✅ Implementation → Bitstream → DRC=0
7. 🔄 上板：LED/数码管验证（C待执行）
```

### 需要依赖 Vivado 综合后才知道的项目（已有答案）

- ~~MAC 组合路径是否满足 100MHz 时序~~ → ✅ **满足**，WNS=+7.212ns，100MHz 时序充裕，无需流水线化
- ~~DSP48E1 是否被成功推断~~ → 待从 utilization report 确认（B/C 导出后可知）
- ~~data_ram 字节使能写入能否推断为 BRAM~~ → 待从 utilization report 确认
