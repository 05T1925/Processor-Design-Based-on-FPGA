...
- B 已补齐 `LW/SW` 与 `BEQ/BNE` 扩展仿真，并打通 **VGA + S1~S5 普通按键**的上板交互演示链路
...
| **LW/SW + BEQ/BNE xsim PASS** | 指令覆盖补强 | `tb_load_store.v` 与 `tb_branch.v` 已由 B 验证通过 |
| **VGA + S1~S5 小游戏骨架** | 上板演示亮点 | 开始页/输入页/结果页闭环，蓝/红/绿边框反馈结果 |
...
> 项目已完成验收。以下旧表格保留为开发过程记录；其中“待完成”条目在最终交付时均已关闭。答辩或复盘时，必须明确区分：多周期/VGA 主板实测、流水线测试实测、以及早期模型估算，不能混用。
...
| 进阶层次 | **90%** | SoC集成 + perf MMIO + CPI/吞吐量量化评估 + LW/SW/BEQ/BNE 扩展仿真完成 |
| 拓展层次 | **95%** | MAC 验证 + 点积对比(speedup=1.1481) + PPA 模板 + **BTB 分支预测** + **VGA/按键小游戏骨架** + **12文档答辩数据体系** 完成 |
