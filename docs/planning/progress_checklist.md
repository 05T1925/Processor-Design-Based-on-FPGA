...
**最终归档（2026-07-13）**：MAC 基础、验证、点积对比、流水线实现/测试与 PPA 工作均已完成并用于验收。历史段落中的“待 Vivado”仅反映当时的计划状态；引用具体 PPA 数值时，必须使用对应最终实现报告，而不能以早期估算值代替。
...
进阶层次 ████████████████████  100% (系统集成、性能计数、VGA/板级演示、流水线验证和 PPA 均完成)
拓展层次 ████████████████████  100% (MAC、流水线、BTB、游戏与性能可视化完成)
```
