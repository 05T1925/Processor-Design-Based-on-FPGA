...
- `docs/design/interfaces.md` 中涉及板级端口的部分需要人工确认，待老师提供的 Minisys 官方 `.xdc` 到位后核对板级端口。
- `docs/design/board_demo.md` 中涉及板级端口命名的部分需要人工确认，待官方 `.xdc` 到位后确认端口命名。
- 最终板级端口名必须与官方约束一致。如果官方 `.xdc` 与当前暂定命名不同，由 `minisys_top` 做统一映射。
- 目前尚未下载安装老师提供的软件安装包，因此 Minisys 官方 `.xdc` 是否已到位仍不确定；组长安装好环境后再确认。
...
| `minisys_top` | 板级顶层，等待官方 `.xdc` 后统一端口映射 |
...
| `constraints/minisys.xdc` | Minisys 约束文件，待官方 `.xdc` 核对 |
