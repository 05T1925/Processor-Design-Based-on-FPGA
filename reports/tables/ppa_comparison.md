...
日期：2026-07-13（最终归档同步）
负责人：D 王博生 + 流水线实现队友 + AI 辅助

> 最终状态：流水线测试、上板与 PPA 工作已完成。下表中的“待测”是早期模板遗留，不表示项目未完成；最终答辩/验收数字必须从对应 `CPU_MODE=5` 的 Vivado implementation utilization/timing 报告摘录。不要将本文件后续的模型估计值写成实测结果。
