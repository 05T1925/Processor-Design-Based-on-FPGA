...
五级流水线、forwarding、load-use stall、branch flush 已完成并通过 Vivado XSim 实测。基础程序为 10 cycle / 6 instret（CPI=1.67）；冒险程序为 28 cycle / 17 instret（CPI=1.65），实测触发 EX/MEM 前递 6 次、MEM/WB 前递 4 次、load-use stall 1 次、branch flush 1 次、JAL flush 2 次。**BTB 动态分支预测（16条目 2-bit）已完成**；BTB 专项正确率仍需独立仿真。UART 输出统计为后续冲刺目标。
...
- ✅ **五级流水线 XSim 实测通过**（2026-07-12）：基础测试 PASS（10 cycle / 6 instret / CPI 1.67）；冒险测试 PASS（28 / 17 / CPI 1.65），覆盖两级前递、1-cycle load-use stall、taken/not-taken branch 与 JAL flush。测试中修复 WB→ID 旁路、SW 数据前递寄存器号和 EX 分支冲刷范围。
...
- 🔄 待完成：流水线 Vivado 综合 PPA 导出（B/C）、BTB/JALR 专项仿真与预测正确率统计。LED/数码管 CPU-MMIO 上板交互已验证。
